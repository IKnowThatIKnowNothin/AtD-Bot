...
class Duel:
    ...
    def run_round(self, dueler1, dueler2, roundCount):
        ...
        roundmessage += "**Damage** Roll: {} ({}{:+})\n \n".format(dmgDealt, dmgDealt - winner.extradmg,
                                                                   winner.extradmg)
        if (win_raw == 20):
            roundmessage += "\n\n {} has a critical hit \n\n".format(winner.name)
            if self.LIVE_STEEL:
                roundmessage = loser.apply_injury(roundmessage)
        ...
        if (dueler2.morale <= 0) or (dueler2.morale <= (dueler2.startpoint + dueler2.threshold)) or (
                dueler2.continueFighting == False):
            if (dueler2.morale <= 0):
                print("A1")
        ...
        elif (dueler1.morale <= 0) or (dueler1.morale <= (dueler1.startpoint + dueler1.threshold)) or (
                dueler1.continueFighting == False):
            if (dueler1.morale <= 0):
                print("B1")
            if (dueler1.morale <= (dueler1.startpoint + dueler1.threshold)):
                print("B2")
                print(dueler1.startpoint, " ", dueler1.threshold)
    ...
    def run(self, duelInfo):
        ...
        skills1 = int(duelInfo.group(6)) if duelInfo.group(6) else 0
        skills2 = int(duelInfo.group(12)) if duelInfo.group(12) else 0

        dueler1 = Dueler.Dueler(duelInfo.group(1), int(duelInfo.group(2)), int(duelInfo.group(3)), skills1)
        dueler2 = Dueler.Dueler(duelInfo.group(7), int(duelInfo.group(8)), int(duelInfo.group(9)), skills2)

        dueler1.morale += int(duelInfo.group(4))
        dueler1.extradmg += int(duelInfo.group(5))
        dueler2.morale += int(duelInfo.group(10))
        dueler2.extradmg += int(duelInfo.group(11))
