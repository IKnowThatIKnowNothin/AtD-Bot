...
# Build location_to_region dynamically from Locations.txt
    ...
    def check_chokepoints_in_path(self, path):
        ...
        for i in range(len(path) - 1):
            ...
            if pair == {"Riverlands", "Vale"}:
                notices.append("Movement between the Riverlands and the Vale requires permission to pass through the Bloody Gate. " \
                "Please ping the controller of the Bloody Gate (presumed House Arryn).")

            if pair == {"Riverlands", "North"}:
                notices.append("Movement between the Riverlands and the North requires permission to pass through Moat Cailin. " \
                "Please ping the controller of Moat Cailin (presumed House Stark).")

            if pair == {"Riverlands", "Westerlands"}:
                notices.append("Movement between the Riverlands and the Westerlands requires permission to pass through the Golden Tooth. " \
                "Please ping House Lefford of the Golden Tooth.")

            if pair == {"Stormlands", "Dorne"}:
                notices.append("Movement between the Stormlands and Dorne is assumed to pass through the Boneway or the eastern Prince’s Pass. " \
                "Both House Dondarrion and House Yronwood must be pinged for permission.")

            if pair == {"Reach", "Dorne"}:
                notices.append("Movement between the Reach and Dorne is assumed to pass through the western Prince’s Pass. " \
                "Both House Caron and House Fowler must be pinged for permission.")
